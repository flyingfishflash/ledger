...
package net.flyingfishflash.ledger.nestedset.unit.mem;


public class InMemoryTestHelper implements TestHelper {

  private final InMemoryNestedNodeRepositoryConfiguration<Long, TestNode> config =
      NestedSetTestConfiguration.IN_MEM_CONFIG;

  @Override
  public TestNode findNode(String symbol) {
    System.out.println(config.getNodes());
    return config.getNodes().stream()
        .filter(n -> n.getName().equals(symbol))
        .findFirst()
        .orElse(null);
  }
  ...
  public TestNode getParent(TestNode f) {
    ...
    TestNode parent =
        config.getNodes().stream()
            .filter(n -> n.getId().equals(f.getParentId()))
            .findFirst()
            .orElse(null);
    System.out.println(
        String.format(
  ...
  public void breakTree() {
    config.getNodes().stream()
        .filter(n -> n.getDiscriminator().equals("tree_1"))
        .forEach(
            n -> {
              n.setTreeLevel(0L);
              n.setTreeLeft(0L);
  ...
  public void rollback() {
    ...
    config
        .getNodes()
        .addAll(
            NestedSetTestConfiguration.IN_MEM_NODES.stream()
                .map(TestNode::copy)
                .collect(Collectors.toList()));
  }
}
