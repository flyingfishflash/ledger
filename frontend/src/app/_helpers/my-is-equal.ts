export interface ObjectEqualityState {
  equal: boolean;
  differences: string[];
}

export function myIsEqual(value, other): ObjectEqualityState {
  let myReturn: ObjectEqualityState;
  myReturn = { equal: true, differences: [] };
  console.log(myReturn);

  // Get the value type
  const type = Object.prototype.toString.call(value);
  // let objName = Object.prototype.toString.call(other);

  /*     console.log(type);
        console.log(objName);
        console.log(Object.getOwnPropertyNames(value));
     */
  // If the two objects are not the same type, return false
  if (type !== Object.prototype.toString.call(other)) {
    myReturn.equal = false;
    // console.log(myReturn);
    return myReturn;
  }

  // If items are not an object or array, return false
  if (["[object Array]", "[object Object]"].indexOf(type) < 0) {
    myReturn.equal = false;
    // console.log(myReturn);
    return myReturn;
  }

  // Compare the length of the length of the two items
  const valueLen =
    type === "[object Array]" ? value.length : Object.keys(value).length;
  const otherLen =
    type === "[object Array]" ? other.length : Object.keys(other).length;
  if (valueLen !== otherLen) {
    myReturn.equal = false;
    // console.log(myReturn);
    return myReturn;
  }

  // Compare two items
  const compare = (item1, item2) => {
    console.log(item1, item2);

    // Get the object type
    const itemType = Object.prototype.toString.call(item1);

    // If an object or array, compare recursively
    if (["[object Array]", "[object Object]"].indexOf(itemType) >= 0) {
      if (!myIsEqual(item1, item2)) {
        myReturn.equal = false;
        // console.log(myReturn);
        // return myReturn.equal;
      }
    }

    // Otherwise, do a simple comparison
    else {
      // If the two items are not the same type, return false
      if (itemType !== Object.prototype.toString.call(item2)) {
        myReturn.equal = false;
        // console.log(myReturn);
        // return myReturn.equal;
      }

      // Else if it's a function, convert to a string and compare
      // Otherwise, just compare
      if (itemType === "[object Function]") {
        if (item1.toString() !== item2.toString()) {
          myReturn.equal = false;
          // console.log(myReturn);
          return myReturn.equal;
        }
      } else {
        if (item1 !== item2) {
          myReturn.equal = false;
          // console.log(myReturn);
          return myReturn.equal;
        }
      }
    }
  }; // compare function

  // Compare properties
  if (type === "[object Array]") {
    for (let i = 0; i < valueLen; i++) {
      if (compare(value[i], other[i]) === false) {
        myReturn.equal = false;
        // console.log(myReturn);
        return myReturn;
      }
    }
  } else {
    for (const key in value) {
      console.log(key);
      if (value.hasOwnProperty(key)) {
        if (compare(value[key], other[key]) === false) {
          myReturn.equal = false;
          myReturn.differences.push(key);
          // console.log(myReturn);
          // return myReturn;
        }
      }
    }
  }

  // If nothing failed, return true
  {
    // myReturn.equal = true;
    console.log(myReturn);
    return myReturn;
  }
}

export function isEqual2(value, other): boolean {
  // var isEqual1 = function (value, other) {

  // const myReturn: { equal: boolean, differences: any };

  // Get the value type
  const type = Object.prototype.toString.call(value);

  // If the two objects are not the same type, return false
  if (type !== Object.prototype.toString.call(other)) {
    return false;
  }

  // If items are not an object or array, return false
  if (["[object Array]", "[object Object]"].indexOf(type) < 0) {
    return false;
  }

  // Compare the length of the length of the two items
  const valueLen =
    type === "[object Array]" ? value.length : Object.keys(value).length;
  const otherLen =
    type === "[object Array]" ? other.length : Object.keys(other).length;
  if (valueLen !== otherLen) {
    return false;
  }

  // Compare two items
  const compare = (item1, item2) => {
    // Get the object type
    const itemType = Object.prototype.toString.call(item1);

    // If an object or array, compare recursively
    if (["[object Array]", "[object Object]"].indexOf(itemType) >= 0) {
      if (!myIsEqual(item1, item2)) {
        return false;
      }
    }

    // Otherwise, do a simple comparison
    else {
      // If the two items are not the same type, return false
      if (itemType !== Object.prototype.toString.call(item2)) {
        return false;
      }

      // Else if it's a function, convert to a string and compare
      // Otherwise, just compare
      if (itemType === "[object Function]") {
        if (item1.toString() !== item2.toString()) {
          return false;
        }
      } else {
        if (item1 !== item2) {
          return false;
        }
      }
    }
  };

  // Compare properties
  if (type === "[object Array]") {
    for (let i = 0; i < valueLen; i++) {
      if (compare(value[i], other[i]) === false) {
        return false;
      }
    }
  } else {
    for (const key in value) {
      if (value.hasOwnProperty(key)) {
        if (compare(value[key], other[key]) === false) {
          return false;
        }
      }
    }
  }

  // If nothing failed, return true
  return true;
}
