...
export class ImportComponent implements OnInit, OnDestroy {
  ...
  displayedColumnsCommodities: string[] = ['Component', 'Gnc Count', 'Sent to Adapter', 'Persisted', 'Ignored Templates', 'Ignored Currencies'];
  ...
  constructor(
    ...
    ngOnInit(): void {
      ...
      this.importMessagesSubscription = this.rxStompService.watch('/user/import/status/messages').subscribe((message: Message) => {
        console.log('received a message!');
        this.receivedMessages.push(message.body);
        console.log(message.body);
      });

      this.importCountsSubscription = this.rxStompService.watch('/user/import/status/counts').subscribe((message: Message) => {
  ...
  fetchImportStatusComponentCounts() {
    this.importService.getImportStatus().subscribe(
      (res) => {
        res = res || [];
        console.log(res);
