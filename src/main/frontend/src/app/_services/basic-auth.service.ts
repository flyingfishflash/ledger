...
export class BasicAuthService {
  ...
  signIn(credentials) {
    ...
    return this.http.get<any>(API + '/auth/signin', { headers: headers, withCredentials: true }).pipe(map(user => {
      ...
      u.token = this.createBasicAuthToken(user.response.body.username, credentials.password);
      u.roles = user.response.body.roles;
      window.sessionStorage.setItem(this.STORAGE_KEY_AUTHENTICATED_USER, JSON.stringify(u));
